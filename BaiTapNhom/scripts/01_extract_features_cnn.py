# scripts/01_extract_features_cnn.py
...
SAVE_PATH_DEV_X = os.path.join(DATA_ROOT, "features/cnn/X_dev.npy")
SAVE_PATH_DEV_Y = os.path.join(DATA_ROOT, "features/cnn/y_dev.npy")
SAVE_PATH_EVAL_X = os.path.join(DATA_ROOT, "features/cnn/X_eval.npy")
SAVE_PATH_EVAL_Y = os.path.join(DATA_ROOT, "features/cnn/y_eval.npy")
...
def extract_mfcc_features(audio_dir, df):
    ...
    for fname, label in tqdm(zip(df["fname"], df["label_binary"]), total=len(df)):
        ...
        try:
            ...
            mfcc = mfcc.T  # Shape: (T, N_MFCC) where T is time and N_MFCC is number of MFCC features
            ...
            X.append(mfcc)  # Không flatten MFCC để giữ nguyên chiều (MAX_LEN, N_MFCC)
            y.append(label)
